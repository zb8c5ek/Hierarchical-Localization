...
def get_colmap_executable_path():
    """
        Example usage
        colmap_executable_path = get_colmap_executable_path()
        print(colmap_executable_path)
    Returns
    -------
    ...
    if platform.system() == "Windows":
        ...
        for path in possible_colmap_paths:
            if Path(path).is_file():
                fp_colmap_exe = Path(path)
                break

        # Double-check COLMAP executable path using `which colmap` command
        if fp_colmap_exe is None:
            process = subprocess.run(["which", "colmap"], capture_output=True, text=True)
            output_lines = process.stdout.splitlines()
            if len(output_lines) > 0:
                fp_colmap_exe = Path(output_lines[0])

        if fp_colmap_exe is None:
            raise FileNotFoundError("COLMAP executable not found on Linux system")

    return fp_colmap_exe


def main():
    dp_facade_imgs = Path("E:\datasets\eth3d/facade/facade_undistorted").resolve()
    fp_colmap_exe = get_colmap_executable_path()
